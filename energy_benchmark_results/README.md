...
This data contains the energy of all generated structures for each method before and after optimization and the energy of the ground truth structures for 20 molecular ensembles from GEOMXL (this is very expensive so only a subset). There is frequency data for some structures for validating how well the optimization went, but we decided frequency calculations for all structures would be too slow and ay too memory intensive because we're storing all intermediate structures. Some optimizations failed but very few, those are not included here. I optimized all of the ground truth structures too, but realized they were already optimized so the pre and post optimization numbers should be the same.
